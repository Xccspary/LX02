    ...
    class UserInfo
    {
        ...
        public Model.UserInfo  GetModel(string UserName)
        {
            using(IDbConnection cn =new MySqlConnection(cns))
            {
                ...
                return cn.QueryFirstOrDefault<Model.UserInfo>(sql, new { UserName });
            }
        }
        ...
        public IEnumerable<Model.UserInfoNo> Getpage(Model.Page page)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "wite a as(select row_number() over(order by username) as num, userinfo.* from userinfo)";
        ...
        public int Add(Model.UserInfo user)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "insert info userinfo value(@username,@pageWord,@qq,@email,@type,@userImg);";
        ...
        public int Delete(string username)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "delte from username where username=@username";
